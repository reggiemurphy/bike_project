...
class ScraperDB:
	...
	def insert(self, number, station, lat, lng, status, total_stands, free_stands, free_bikes, weather, updated):
		# Creating cursor.
		c = self.conn.cursor()

		# Statement to insert row into table. 
		c.execute("INSERT IGNORE INTO BikeData VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s)", (int(number), station, float(lat), float(lng), status, int(total_stands), int(free_stands), int(free_bikes), weather, str(updated)))
